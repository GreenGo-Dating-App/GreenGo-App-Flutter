  ...
  .onRun(async (context) => {
    try {
      ...
      const createTrainingDataQuery = `
        CREATE OR REPLACE TABLE \`${DATASET_ID}.churn_training_data\` AS
        WITH user_features AS (
          ...
          FROM \`${DATASET_ID}.user_cohorts\` uc
          LEFT JOIN (
          ...
          LEFT JOIN (
            ...
            FROM \`${DATASET_ID}.subscription_events\`
            GROUP BY user_id
      ...
      await bigquery.query(createTrainingDataQuery);
      console.log('Training data created');

      // Train the model using BigQuery ML
      const trainModelQuery = `
        CREATE OR REPLACE MODEL \`${DATASET_ID}.churn_prediction_model\`
        OPTIONS(
        ...
        SELECT
          ...
          lifetime_value,
          avg_monthly_spend
        FROM \`${DATASET_ID}.churn_training_data\`
      `;

      await bigquery.query(trainModelQuery);
      ...
      const evaluateQuery = `
        SELECT
          ...
          f1_score
        FROM ML.EVALUATE(
          MODEL \`${DATASET_ID}.churn_prediction_model\`,
          (SELECT * FROM \`${DATASET_ID}.churn_training_data\`)
        )
      `;

      const [evaluationResults] = await bigquery.query(evaluateQuery);
...
export const predictChurnDaily = functions.pubsub
  ...
  .onRun(async (context) => {
    try {
      ...
      const [predictions] = await bigquery.query(predictQuery);
...
export const getAtRiskUsers = functions.https.onCall(
  async (data, context) => {
    ...
    if (!customClaims.admin) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only admins can access at-risk users'
      );
    }
...
function getRiskLevel(churnScore: number): 'low' | 'medium' | 'high' | 'critical' {
...
async function analyzeChurnFactors(userId: string): Promise<ChurnFactor[]> {
...
function estimateChurnDate(churnScore: number): Date | undefined {
