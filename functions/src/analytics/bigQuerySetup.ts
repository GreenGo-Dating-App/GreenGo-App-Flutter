...
export const revenueEventsSchema = [
...
export const userCohortsSchema = [
  { name: 'user_id', type: 'STRING', mode: 'REQUIRED' },
...
export const refundEventsSchema = [
...
export const abTestResultsSchema = [
...
export async function initializeBigQuery(): Promise<void> {
  try {
    ...
    // Create tables
    const tables = [
      { name: 'revenue_events', schema: revenueEventsSchema },
      { name: 'subscription_events', schema: subscriptionEventsSchema },
      { name: 'user_cohorts', schema: userCohortsSchema },
      { name: 'refund_events', schema: refundEventsSchema },
      { name: 'ab_test_results', schema: abTestResultsSchema },
      { name: 'tax_records', schema: taxRecordsSchema },
    ];
 ...
 */
export async function insertRevenueEvent(data: {
  eventId: string;
  userId: string;
  eventType: string;
  eventTimestamp: Date;
  ...
  const row = {
    ...
    user_cohort: data.userCohort?.toISOString().split('T')[0],
 ...
 */
export async function insertSubscriptionEvent(data: {
  eventId: string;
 ...
 */
export async function insertRefundEvent(data: {
  refundId: string;
  userId: string;
 ...
 */
export async function upsertUserCohort(data: {
  userId: string;
  cohortDate: Date;
  cohortMonth: string;
  acquisitionChannel: string;
  ...
  await bigquery.query({
    ...
    params: {
      ...
      first_subscription_date: data.firstSubscriptionDate?.toISOString(),
      lifetime_value: data.lifetimeValue,
      total_revenue: data.totalRevenue,
      subscription_months: data.subscriptionMonths,
      is_churned: data.isChurned,
      churn_date: data.churnDate?.toISOString(),
      churn_prediction_score: data.churnPredictionScore,
    },
  });
}

export { bigquery, DATASET_ID };
