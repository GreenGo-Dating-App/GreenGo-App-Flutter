 ...
 */
export const runSecurityAudit = functions
  .runWith({
  ...
  .https.onCall(async (data, context) => {
    ...
    if (!adminData.permissions.includes('run_security_audit')) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Insufficient permissions to run security audit'
      );
    }
...
export const getSecurityAuditReport = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (!adminDoc.exists) {
      throw new functions.https.HttpsError(
        'permission-denied',
        'Only admins can view security audit reports'
      );
    }
 ...
 * Generate PDF Report
 */
async function generatePDFReport(report: any, reportId: string): Promise<string> {
  try {
 ...
 */
async function notifyAdminsOfCriticalIssues(report: any): Promise<void> {
  try {
 ...
 */
async function notifyAdminsOfAuditResults(report: any, reportId: string): Promise<void> {
  try {
    const adminsSnapshot = await firestore
      .collection('admin_users')
      .where('role', 'in', ['super_admin', 'admin'])
