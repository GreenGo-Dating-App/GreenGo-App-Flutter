 ...
 */
async function handleSubscriptionRecovered(subscriptionId: string, subscription: any) {
 ...
 */
async function handleSubscriptionRenewed(subscriptionId: string, subscription: any) {
  const now = new Date();
  ...
  await firestore.collection('purchases').add({
    ...
    price: SUBSCRIPTION_TIERS[subscription.tier as keyof typeof SUBSCRIPTION_TIERS].price,
 ...
 */
async function handleSubscriptionCanceled(subscriptionId: string, subscription: any) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'cancelled',
    autoRenew: false,
  ...
  await firestore.collection('notifications').add({
    ...
    message: `Your ${subscription.tier} subscription has been cancelled. You'll have access until ${subscription.endDate?.toDate().toLocaleDateString()}.`,
    createdAt: admin.firestore.FieldValue.serverTimestamp(),
    isRead: false,
  });
 ...
 */
async function handleSubscriptionPurchased(subscriptionId: string, subscription: any) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'active',
    verifiedAt: admin.firestore.FieldValue.serverTimestamp(),
    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
 ...
 */
async function handleSubscriptionOnHold(subscriptionId: string, subscription: any) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'suspended',
    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
  });
 ...
 */
async function handleGracePeriodStarted(subscriptionId: string, subscription: any) {
 ...
 */
async function handleSubscriptionRestarted(subscriptionId: string, subscription: any) {
  const now = new Date();
 ...
 */
async function handlePriceChangeConfirmed(subscriptionId: string, subscription: any) {
 ...
 */
async function handleSubscriptionRevoked(subscriptionId: string, subscription: any) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'refunded',
 ...
 */
async function handleSubscriptionExpired(subscriptionId: string, subscription: any) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'expired',
    autoRenew: false,
    updatedAt: admin.firestore.FieldValue.serverTimestamp(),
 ...
 * Handle refund
 */
async function handleRefund(subscriptionId: string, subscription: any) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'refunded',
    endDate: admin.firestore.FieldValue.serverTimestamp(),
    autoRenew: false,
 ...
 */
async function endGracePeriod(subscriptionId: string) {
  await firestore.collection('subscriptions').doc(subscriptionId).update({
    status: 'expired',
    inGracePeriod: false,
    gracePeriodEndDate: null,
  ...
  await downgradeToBasic(subscription.data()?.userId);
 ...
 */
async function downgradeToBasic(userId: string) {
