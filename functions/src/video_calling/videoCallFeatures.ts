...
export const toggleBeautyMode = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const beautySettings = {
      ...
      brightening: settings?.brightening || 0.3,
      eyeEnhancement: settings?.eyeEnhancement || 0.4,
      faceSlimming: settings?.faceSlimming || 0.2,
    };
...
export const startScreenSharing = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (callData.features?.screenSharingUserId && callData.features.screenSharingUserId !== userId) {
...
export const stopScreenSharing = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    if (callData.features?.screenSharingUserId !== userId) {
