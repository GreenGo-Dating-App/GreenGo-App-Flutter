...
const BROADCAST_MESSAGES: Record<string, Record<string, { title: string; body: string }>> = {
...
export const LOCALIZED_MESSAGES = BROADCAST_MESSAGES;
...
interface BroadcastNotificationRequest {
  messageType?: 'welcome' | 'accountApproved' | 'launchReminder' | 'launchDay' | 'custom';
  customTitle?: Record<string, string>; // Language code to title
...
function getAccessDateForTier(tier: SubscriptionTier): Date {
  if (tier === SubscriptionTier.SILVER || tier === SubscriptionTier.GOLD || tier === SubscriptionTier.PLATINUM) {
...
export const approveUser = onCall<ApproveUserRequest>(
  ...
  async (request) => {
    try {
      ...
      const tier = userData.membershipTier as SubscriptionTier || SubscriptionTier.BASIC;
  ...
  async (request) => {
    try {
      ...
      await db.collection('users').doc(userId).update({
        ...
        tierUpdatedBy: request.auth?.uid,
        updatedAt: FieldValue.serverTimestamp(),
      });
...
export const getPendingUsers = onCall<GetPendingUsersRequest>(
  ...
  async (request) => {
    try {
      ...
      logInfo('Fetching pending users');
      ...
      const pendingUsers = snapshot.docs.map(doc => ({
        ...
        membershipTier: doc.data().membershipTier || SubscriptionTier.BASIC,
...
export const bulkApproveUsers = onCall<{ userIds: string[]; notify?: boolean }>(
  {
    memory: '512MiB',
    timeoutSeconds: 300,
  },
  ...
  async (request) => {
    try {
      ...
      switch (targetAudience) {
        case 'approved':
          query = query.where('approvalStatus', '==', ApprovalStatus.APPROVED);
          break;
        case 'premium':
          ...
          break;
        case 'basic':
          query = query.where('membershipTier', '==', SubscriptionTier.BASIC);
          break;
        case 'pending':
          query = query.where('approvalStatus', '==', ApprovalStatus.PENDING);
          break;
      ...
      while (true) {
        ...
        for (const doc of snapshot.docs) {
          ...
          if (messageType === 'custom') {
            title = customTitle?.[userLang] || customTitle?.en || 'GreenGoChat Notification';
            body = customBody?.[userLang] || customBody?.en || 'You have a new notification';
          } else {
            const predefinedMessages = BROADCAST_MESSAGES[messageType];
            const message = predefinedMessages?.[userLang] || predefinedMessages?.en;
            title = message?.title || 'GreenGoChat';
          ...
          try {
            ...
            response.responses.forEach((resp, idx) => {
              if (resp.error &&
                ...
                const failedToken = (fcmBatch[idx] as admin.messaging.TokenMessage).token;
                if (failedToken) {
            ...
            logError('Error sending FCM batch:', err);
        ...
        logInfo(`Broadcast progress: ${totalTargeted} users processed, ${successCount} sent`);
...
export const getMvpAccessStats = onCall(
  ...
  async (request) => {
    try {
      ...
      logInfo('Fetching MVP access stats');
      ...
      const [
        ...
        silverSnapshot,
        basicSnapshot,
        notificationsEnabledSnapshot,
      ] = await Promise.all([
        db.collection('users').count().get(),
        db.collection('users').where('approvalStatus', '==', ApprovalStatus.PENDING).count().get(),
        db.collection('users').where('approvalStatus', '==', ApprovalStatus.APPROVED).count().get(),
        db.collection('users').where('approvalStatus', '==', ApprovalStatus.REJECTED).count().get(),
        db.collection('users').where('membershipTier', '==', SubscriptionTier.PLATINUM).count().get(),
        db.collection('users').where('membershipTier', '==', SubscriptionTier.GOLD).count().get(),
        db.collection('users').where('membershipTier', '==', SubscriptionTier.SILVER).count().get(),
        db.collection('users').where('membershipTier', '==', SubscriptionTier.BASIC).count().get(),
        db.collection('users').where('notificationsEnabled', '==', true).count().get(),
      ]);
