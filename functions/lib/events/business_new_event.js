...
async function fanOutNewEventToFollowers(eventId, eventData, eventRef) {
    ...
    const claimed = await db.runTransaction(async (txn) => {
        ...
        if (!fresh.exists)
            return false;
        if (((_a = fresh.data()) === null || _a === void 0 ? void 0 : _a.pushedToFollowers) === true)
            return false;
        txn.update(eventRef, {
    ...
    while (true) {
        ...
        const userDocs = await Promise.all(followerIds.map((uid) => db.collection('users').doc(uid).get()));
        const tokens = [];
            ...
            try {
                await admin.messaging().sendEachForMulticast({
                    ...
                    android: {
                        ...
                        notification: Object.assign({ sound: 'default', channelId: 'greengo_notifications', priority: 'high', tag: `event_new_${eventId}` }, (eventImage ? { imageUrl: eventImage } : {})),
                    },
                    apns: {
...
//# sourceMappingURL=business_new_event.js.map