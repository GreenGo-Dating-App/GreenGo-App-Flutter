...
const PLAY_URL = 'https://play.google.com/store/apps/details?id=com.greengochat.greengochatapp';
...
function millis(v) {
    if (v instanceof firebaseAdmin_1.admin.firestore.Timestamp)
        return v.toMillis();
    return 0;
}
async function loadPreview(kind, id) {
    ...
    const generic = {
        ...
        id,
        title: 'GreenGo',
        description: 'Discover people, cultures, events and communities around the world.',
        imageUrl: null,
        version: 0,
        shareable: false,
    };
...
async function renderImage(p) {
    ...
    try {
        ...
        return await (0, sharp_1.default)(Buffer.from(resp.data))
