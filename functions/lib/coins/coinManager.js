...
exports.verifyAppStoreCoinPurchase = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const verified = true; // Placeholder
        if (!verified) {
            throw new functions.https.HttpsError('invalid-argument', 'Receipt verification failed');
        }
        const coinAmount = getCoinAmountFromPackage(packageId);
...
exports.grantMonthlyAllowances = functions.pubsub
    ...
    try {
        ...
        for (const doc of subscriptionsSnapshot.docs) {
            ...
            if (tier === 'basic')
                continue;
            const allowanceAmount = MONTHLY_ALLOWANCE[tier];
            if (!allowanceAmount)
                continue;
...
exports.processExpiredCoins = functions.pubsub
    ...
    try {
        ...
        for (const doc of balancesSnapshot.docs) {
            ...
            let expiredAmount = 0;
            const updatedBatches = coinBatches.filter((batch) => {
    ...
    try {
        ...
        for (const doc of balancesSnapshot.docs) {
            ...
            if (expiringCoins > 0 && earliestExpiration) {
                const daysUntilExpiration = Math.ceil((earliestExpiration.getTime() - now.getTime()) / (1000 * 60 * 60 * 24));
                await firestore.collection('notifications').add({
                    userId,
