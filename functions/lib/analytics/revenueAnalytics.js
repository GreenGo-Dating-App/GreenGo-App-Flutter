...
exports.getRevenueDashboard = functions.https.onCall(async (data, context) => {
    ...
    if (!customClaims.admin) {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can access revenue dashboard');
    }
    const { startDate, endDate } = data;
    try {
        const [dailyRevenue, weeklyRevenue, monthlyRevenue, revenueBySource, summary,] = await Promise.all([
      ...
      SELECT
        ...
        SUM(CASE
          WHEN event_type = 'upgraded' THEN price - COALESCE(
            (SELECT price FROM \`${bigQuerySetup_1.DATASET_ID}.subscription_events\` s2
        ...
        SUM(CASE
          ...
          ELSE 0
        END) as churned_mrr
      FROM \`${bigQuerySetup_1.DATASET_ID}.subscription_events\` s1
      WHERE DATE(event_timestamp) >= DATE_SUB(CURRENT_DATE(), INTERVAL 12 MONTH)
      GROUP BY month
    )
    SELECT
...
exports.exportRevenueData = functions.https.onCall(async (data, context) => {
    ...
    const customClaims = context.auth.token;
    if (!customClaims.admin) {
        throw new functions.https.HttpsError('permission-denied', 'Only admins can export revenue data');
    }
