...
exports.batchChurnPrediction = exports.predictUserChurn = exports.calculateCohortRetention = exports.createUserCohort = exports.calculateUserSegment = void 0;
 ...
 */
exports.calculateUserSegment = functions.https.onCall((0, monitoring_1.monitored)("calculateUserSegment", async (data, context) => {
    var _a;
    if (!context.auth) {
    ...
    try {
        ...
        const lastActiveAt = ((_a = userData.lastActiveAt) === null || _a === void 0 ? void 0 : _a.toDate()) || new Date(0);
        const daysSinceLastActive = Math.floor((now.getTime() - lastActiveAt.getTime()) / (24 * 60 * 60 * 1000));
        // Total days active
        const accountCreatedAt = userData.createdAt.toDate();
        const totalDaysActive = Math.floor((now.getTime() - accountCreatedAt.getTime()) / (24 * 60 * 60 * 1000));
...
function calculateEngagementScore(metrics) {
    ...
    if (metrics.sessionsPerWeek >= 10)
        score += 30;
    else if (metrics.sessionsPerWeek >= 5)
        score += 20;
    else if (metrics.sessionsPerWeek >= 2)
        score += 10;
    // Messages (max 25 points)
    if (metrics.messagesPerWeek >= 20)
        score += 25;
    else if (metrics.messagesPerWeek >= 10)
        score += 15;
    else if (metrics.messagesPerWeek >= 5)
        score += 10;
    // Matches (max 20 points)
    if (metrics.matchesPerWeek >= 5)
        score += 20;
    else if (metrics.matchesPerWeek >= 2)
        score += 10;
    else if (metrics.matchesPerWeek >= 1)
        score += 5;
    // Session duration (max 15 points)
    if (metrics.avgSessionDuration >= 20)
        score += 15;
    else if (metrics.avgSessionDuration >= 10)
        score += 10;
    else if (metrics.avgSessionDuration >= 5)
        score += 5;
    // Recency (max 10 points) - inverse
    if (metrics.daysSinceLastActive === 0)
        score += 10;
    else if (metrics.daysSinceLastActive <= 3)
        score += 7;
 ...
 */
exports.createUserCohort = functions.pubsub
    .schedule('0 2 * * *') // Daily at 2 AM
    .onRun((0, monitoring_1.monitored)("createUserCohort", async (context) => {
    try {
 ...
 */
exports.calculateCohortRetention = functions.pubsub
    .schedule('0 3 * * *') // Daily at 3 AM
    .onRun((0, monitoring_1.monitored)("calculateCohortRetention", async (context) => {
    try {
        // Get all cohorts
        ...
        for (const cohortDoc of cohortsSnapshot.docs) {
            ...
            const now = new Date();
            const daysSinceAcquisition = Math.floor((now.getTime() - acquisitionDate.getTime()) / (24 * 60 * 60 * 1000));
 ...
 */
exports.predictUserChurn = functions.https.onCall((0, monitoring_1.monitored)("predictUserChurn", async (data, context) => {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const userId = context.auth.uid;
    try {
        ...
        return {
            ...
            riskLevel,
            contributingFactors: churnFactors,
            predictedChurnDate: (predictedChurnDate === null || predictedChurnDate === void 0 ? void 0 : predictedChurnDate.toISOString()) || null,
            recommendedInterventions: interventions,
        };
    }
    catch (error) {
 ...
 */
exports.batchChurnPrediction = functions.pubsub
    .schedule('0 4 * * *') // Daily at 4 AM
    .onRun((0, monitoring_1.monitored)("batchChurnPrediction", async (context) => {
    try {
...
//# sourceMappingURL=userSegmentation.js.map