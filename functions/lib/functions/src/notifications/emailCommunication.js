...
exports.startWelcomeEmailSeries = functions.firestore
    ...
    try {
        ...
        await exports.sendTransactionalEmail.run({
            userId,
            emailType: 'welcome',
            templateData: {
...
exports.processWelcomeEmailSeries = functions.pubsub
    ...
    try {
        ...
        for (const seriesDoc of seriesSnapshot.docs) {
            ...
            const daysSinceStart = Math.floor((Date.now() - startedAt.getTime()) / (24 * 60 * 60 * 1000));
            ...
            // Check if completed
            const allSent = series.steps.every((s) => s.isSent);
            if (allSent) {
    ...
    try {
        ...
        for (const userDoc of dormantUsersSnapshot.docs) {
            ...
            const personalizationData = {
                ...
                daysSinceLastActive: Math.floor((Date.now() - userData.lastActiveAt.toMillis()) / (24 * 60 * 60 * 1000)),
            };
...
//# sourceMappingURL=emailCommunication.js.map