 ...
 */
async function verifyModeratorPermission(context) {
    if (!context.auth) {
        throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
    }
    const customClaims = context.auth.token;
    if (!customClaims.moderator && !customClaims.admin) {
        throw new functions.https.HttpsError('permission-denied', 'Only moderators can access moderation queue');
    }
}
/**
 * Log Moderation Action Helper
 */
async function logModerationAction(moderatorId, queueId, action, notes) {
...
exports.getModerationQueue = functions.https.onCall(async (data, context) => {
    ...
    try {
        let query = firestore.collection('moderation_queue');
        // Filter by status
        if (status) {
...
exports.getModerationReviewItem = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        const userContext = {
            ...
            age: userData.age,
            photoUrl: ((_a = userData.photos) === null || _a === void 0 ? void 0 : _a[0]) || null,
        ...
        const suggestedActions = generateSuggestedActions(queueData, userContext, relatedReports);
        return {
            queueId,
            itemType: queueData.itemType,
            content: {
                itemId: queueData.itemId,
                text: (content === null || content === void 0 ? void 0 : content.description) || (content === null || content === void 0 ? void 0 : content.text) || null,
                photoUrls: (content === null || content === void 0 ? void 0 : content.screenshotUrls) || (content === null || content === void 0 ? void 0 : content.photoUrls) || [],
                additionalData: content || {},
                createdAt: (content === null || content === void 0 ? void 0 : content.createdAt) || queueData.addedAt,
            },
            userContext,
            relatedReports,
            history,
 ...
 * Generate AI Suggested Actions
 */
function generateSuggestedActions(queueData, userContext, relatedReports) {
    const suggestions = [];
 ...
 */
async function approveContent(queueData) {
 ...
 */
async function dismissReport(queueData) {
    for (const reportId of queueData.relatedReportIds) {
        await firestore.collection('user_reports').doc(reportId).update({
            status: 'dismissed',
        });
 ...
 */
async function removeContent(queueData) {
 ...
 */
async function issueWarningToUser(userId, queueData, notes) {
 ...
 * Helper: Suspend User
 */
async function suspendUser(userId, reason, durationDays) {
...
exports.executeBulkModeration = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        for (const queueId of queueIds) {
            try {
                await exports.takeModerationAction.run({ queueId, action, notes }, context);
