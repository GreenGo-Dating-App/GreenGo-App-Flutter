    ...
    try {
        ...
        let users = usersSnapshot.docs.map(doc => {
            ...
            return {
                ...
                email: data.email,
                age: data.age,
                photoUrl: ((_a = data.photos) === null || _a === void 0 ? void 0 : _a[0]) || null,
        ...
        if (query && query.trim()) {
            ...
            users = users.map(user => {
                const searchText = query.toLowerCase();
                let score = 0;
                if (user.email.toLowerCase() === searchText)
                    score = 1.0;
                else if (user.displayName.toLowerCase() === searchText)
                    score = 0.9;
                else if (user.email.toLowerCase().includes(searchText))
                    score = 0.7;
                else if (user.displayName.toLowerCase().includes(searchText))
                    score = 0.6;
                else
                    score = 0.3;
                return Object.assign(Object.assign({}, user), { matchScore: score });
            });
            // Sort by match score
...
exports.getDetailedUserProfile = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'viewedUserProfile', 'user', userId, { userId });
        return profile;
    }
    catch (error) {
...
exports.editUserProfile = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'editedUserProfile', 'user', userId, { updates: sanitizedUpdates });
        return { success: true };
    }
    catch (error) {
...
exports.banUserAccount = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'bannedUser', 'user', userId, { reason });
        return { success: true };
    }
    catch (error) {
...
exports.unbanUserAccount = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'unbannedUser', 'user', userId, { userId });
        return { success: true };
    }
    catch (error) {
...
exports.deleteUserAccount = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'deletedUser', 'user', userId, { reason });
        return { success: true };
    }
    catch (error) {
...
exports.overrideUserSubscription = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'overrodeSubscription', 'subscription', subscriptionRef.id, { userId, tier, durationDays });
        return {
            success: true,
            subscriptionId: subscriptionRef.id,
            expirationDate,
...
exports.adjustUserCoins = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'adjustedCoins', 'user', userId, { amount, reason });
        return { success: true };
    }
    catch (error) {
...
exports.sendUserNotification = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        // Log admin action
        await logAdminAction(context.auth.uid, 'sentNotification', 'user', userId, { subject, type });
...
exports.impersonateUser = functions.https.onCall(async (data, context) => {
    ...
    try {
        ...
        await logAdminAction(context.auth.uid, 'impersonatedUser', 'user', userId, { userId, timestamp: new Date().toISOString() });
        return {
            success: true,
            customToken,
...
exports.adminBulkDeleteUsers = functions
    ...
    if (userIds.length > 1000) {
        throw new functions.https.HttpsError('invalid-argument', 'Cannot delete more than 1000 users at once');
    }
    try {
        ...
        for (let i = 0; i < userIds.length; i += checkBatchSize) {
            ...
            const checkResults = await Promise.allSettled(batch.map(async (uid) => {
                try {
        ...
        const deletableUserIds = userIds.filter((uid) => !protectedUserIds.has(uid));
        ...
        await logAdminAction(context.auth.uid, 'bulkDeletedUsers', 'users', `bulk_${deletableUserIds.length}`, {
            reason,
            totalRequested: userIds.length,
            deletedCount,
            skippedProtected: protectedUserIds.size,
