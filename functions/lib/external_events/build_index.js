...
exports.runBuildExternalIndexNow = void 0;
...
function compact(id, d) {
    const out = { id };
    // Keep only the fields the app reads; drop nullish to keep shards small.
    const keys = [
        'title', 'bookingUrl', 'imageUrl', 'category', 'city', 'country',
        'fromPrice', 'currency', 'rating', 'reviewCount', 'durationMinutes',
        'lat', 'lng', 'startDate',
    ];
    for (const k of keys) {
        const v = d[k];
...
async function buildSourceIndex(source) {
    ...
    for (;;) {
        ...
        for (const doc of snap.docs)
            records.push(compact(doc.id, doc.data()));
    ...
    console.log(`buildSourceIndex(${source}): ${records.length} events → ${shardCount} shards.`);
...
exports.runBuildExternalIndexNow = (0, https_1.onRequest)({
    timeoutSeconds: 540,
    memory: '512MiB',
    secrets: [VIATOR_API_KEY, TICKETMASTER_API_KEY],
}, async (req, res) => {
    const token = req.query.token;
    const valid = token === VIATOR_API_KEY.value() || token === TICKETMASTER_API_KEY.value();
    if (!token || !valid) {
...
//# sourceMappingURL=build_index.js.map