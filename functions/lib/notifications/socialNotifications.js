 ...
 */
async function emit(recipientId, type, title, body, data, actor, dedupKey) {
    var _a;
    ...
    try {
        ...
        if (token) {
            await admin.messaging().send({
                token,
                notification: Object.assign({ title: `${actor.name} ${title}`, body }, (actor.photo ? { imageUrl: actor.photo } : {})),
                data,
                android: {
                    priority: 'high',
...
exports.onCommunityMemberJoined = (0, firestore_1.onDocumentCreated)('communities/{communityId}/members/{userId}', (0, monitoring_1.monitored)('onCommunityMemberJoined', async (event) => {
...
exports.onEventAttendeeJoined = (0, firestore_1.onDocumentCreated)('events/{eventId}/attendees/{userId}', (0, monitoring_1.monitored)('onEventAttendeeJoined', async (event) => {
    var _a, _b;
...
exports.onBusinessFollowed = (0, firestore_1.onDocumentCreated)('business_followers/{businessId}/followers/{userId}', (0, monitoring_1.monitored)('onBusinessFollowed', async (event) => {
...
exports.onBusinessRated = (0, firestore_1.onDocumentCreated)('business_ratings/{businessId}/ratings/{userId}', (0, monitoring_1.monitored)('onBusinessRated', async (event) => {
    var _a, _b;
    const businessId = event.params.businessId;
    const userId = event.params.userId;
    if (businessId === userId)
...
exports.onEventLiked = (0, firestore_1.onDocumentCreated)('events/{eventId}/likes/{userId}', (0, monitoring_1.monitored)('onEventLiked', async (event) => {
    const eventId = event.params.eventId;
    const userId = event.params.userId;
    const eSnap = await db.collection('events').doc(eventId).get();
    const e = eSnap.data();
