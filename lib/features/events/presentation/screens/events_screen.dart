...
class _EventsScreenState extends State<EventsScreen>
  ...
  // ---- Experiences tab (Tiqets museums/attractions + Viator tours) ----
  ...
  List<ExternalEvent> _filterExperiences(List<ExternalEvent> items) {
    final q = _searchQuery.toLowerCase();
  ...
  Widget _buildExperiencesTab() {
    return FutureBuilder<List<ExternalEvent>>(
      future: ExternalEventsDataSource().getExperiences(),
      builder: (context, snap) {
        if (snap.connectionState == ConnectionState.waiting) {
          return const Center(
              child: CircularProgressIndicator(color: AppColors.richGold));
        }
        final items = _filterExperiences(snap.data ?? const []);
        if (items.isEmpty) {
          return Center(
            child: Text(
              AppLocalizations.of(context)!.eventsNoEventsFound,
        ...
        if (_gridView) {
          return GridView.builder(
            ...
            itemBuilder: (context, i) => _buildExperienceGridTile(items[i]),
          );
        }
        return ListView.builder(
          padding: const EdgeInsets.all(12),
          itemCount: items.length,
          itemBuilder: (context, i) => _buildExperienceCard(items[i]),
        );
      },
    );
  }
  ...
  Widget _sourceBadge(ExternalEvent e) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: (isTiqets ? Colors.purple : Colors.teal).withOpacity(0.9),
  ...
  String _priceText(ExternalEvent e) {
    if (e.fromPrice == null) return '';
    final l10n = AppLocalizations.of(context)!;
    final cur = e.currency ?? '';
    return '${l10n.eventsFromPrice} $cur ${e.fromPrice!.toStringAsFixed(0)}';
  }

  Widget _buildExperienceCard(ExternalEvent e) {
    return GestureDetector(
      onTap: () => _openBooking(e.bookingUrl),
      child: Container(
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      Expanded(
                        child: Text(
                          [e.city, e.country].where((s) => s != null).join(', '),
                          maxLines: 1,
                          overflow: TextOverflow.ellipsis,
                          style: const TextStyle(
                    ...
                    children: [
                      Text(_priceText(e),
                          style: const TextStyle(
                              ...
                              fontWeight: FontWeight.w600)),
                      ElevatedButton.icon(
                        onPressed: () => _openBooking(e.bookingUrl),
  ...
  Widget _buildExperienceGridTile(ExternalEvent e) {
    return GestureDetector(
      onTap: () => _openBooking(e.bookingUrl),
      child: ClipRRect(
        borderRadius: BorderRadius.circular(12),
                  ...
                  children: [
                    ...
                    Row(
                      children: [
                        ...
                        if (e.fromPrice != null)
                          Text('${e.currency ?? ''}${e.fromPrice!.toStringAsFixed(0)}',
                              style: const TextStyle(
                                  color: AppColors.richGold,
                                  fontSize: 10,
                                  fontWeight: FontWeight.bold)),
