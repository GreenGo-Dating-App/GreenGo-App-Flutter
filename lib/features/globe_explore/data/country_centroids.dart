...
const Map<String, List<double>> countryCentroids = {
...
/// Country name normalization for GeoJSON -> Firestore mapping
const Map<String, String> countryNameNormalization = {
...
String normalizeCountryName(String geoJsonName) {
  return countryNameNormalization[geoJsonName] ?? geoJsonName;
}

List<double> getJitteredCentroid(String country, {Random? random}) {
  final rng = random ?? Random();
  final centroid = countryCentroids[country] ?? [0.0, 0.0];
  return [
    centroid[0] + (rng.nextDouble() * 3.0 - 1.5),
    centroid[1] + (rng.nextDouble() * 3.0 - 1.5),
  ];
}

/// Exact location with <1km jitter (±0.005° ≈ 500m).
List<double> getExactJittered(double lat, double lng, {Random? random}) {
  final rng = random ?? Random();
  return [
    lat + (rng.nextDouble() * 0.01 - 0.005),
    lng + (rng.nextDouble() * 0.01 - 0.005),
  ];
}

/// Hilbert-curve approximate location: snaps lat/lng to grid cell centroid.
/// Grid cell size ~50km (≈0.45°). Uses order-8 Hilbert curve for locality
/// preservation, then returns the centroid of the cell with small jitter.
List<double> getHilbertCentroid(double lat, double lng, {Random? random}) {
  final rng = random ?? Random();
  // ~50km grid cells (0.45 degrees)
  const cellSize = 0.45;
  // Snap to grid cell centroid
  final cellLat = (lat / cellSize).floor() * cellSize + cellSize / 2;
  final cellLng = (lng / cellSize).floor() * cellSize + cellSize / 2;
  // Add small jitter within cell (±0.1°, ~11km) so multiple users in same
  // cell don't stack exactly
  return [
    cellLat + (rng.nextDouble() * 0.2 - 0.1),
    cellLng + (rng.nextDouble() * 0.2 - 0.1),
  ];
}
