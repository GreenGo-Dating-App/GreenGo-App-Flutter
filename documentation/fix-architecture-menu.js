...
const architecturePages = [
    ...
    '18-caching.html',
    '19-real-time.html',
    '20-testing-architecture.html'
];

const pagesDir = path.join(__dirname, 'pages');

architecturePages.forEach(filename => {
    const filepath = path.join(pagesDir, filename);

    if (fs.existsSync(filepath)) {
        let content = fs.readFileSync(filepath, 'utf8');

        // Replace the existing nav-menu with the full menu
        const navMenuRegex = /<ul class="nav-menu" id="navMenu">[\s\S]*?<\/ul>\s*<\/nav>/;
        content = content.replace(navMenuRegex, fullNavMenu + '\n    </nav>');

        fs.writeFileSync(filepath, content);
        console.log(`Fixed menu: ${filename}`);
    } else {
        console.log(`File not found: ${filename}`);
    }
});

console.log('\nAll architecture pages have been updated with the complete navigation menu!');
